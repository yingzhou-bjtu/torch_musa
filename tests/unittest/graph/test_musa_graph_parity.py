...
def test_second_capture_begin_after_inference_mode():
    static_in = torch.ones(8, device=DEVICE)
    ...
    fresh_in = torch.ones(8, device=DEVICE)
    fresh_out = torch.empty_like(fresh_in)
    third = torch.musa.MUSAGraph()
    with torch.no_grad():
        with torch.musa.graph(third):
            fresh_out.copy_(fresh_in)
            fresh_out.add_(1)
            fresh_out.mul_(2)
    fresh_in.fill_(3)
    third.replay()
    torch.musa.synchronize()
    assert testing.DefaultComparator(fresh_out.cpu(), torch.full((8,), 8.0))
